...
function ensureData() {
  ...
  if (!fs.existsSync(USERS_FILE)) {
    fs.writeFileSync(USERS_FILE, JSON.stringify({ users: [{
      ...
      bio:'Platform Administrator', status:'available'
    }] }, null, 2));
  }
...
app.post('/api/register', (req,res) => {
  const {username,name,email,password,color,avatar}=req.body;
  const db=loadJson(USERS_FILE,{users:[]});
  ...
  const newUser={
    ...
    online:true,passwordReset:false,bio:'',status:'available',lastSeen:new Date().toISOString()
  };
...
app.post('/api/update-prefs',(req,res)=>{
  const {userId,lang,theme,bio,status}=req.body;
  const db=loadJson(USERS_FILE,{users:[]});
  const user=db.users.find(u=>u.id===userId);
  if (!user) return res.json({ok:false,error:'not_found'});
  ...
  saveJson(USERS_FILE,db);
  broadcast({type:'user_updated',userId,bio:user.bio,status:user.status});
  res.json({ok:true});
});
...
app.post('/api/messages',(req,res)=>{
  const {roomId,userId,username,name,color,avatar,text,fileUrl,fileName,fileType,replyTo}=req.body;
  ...
  const msg={
    id:Date.now(),roomId,userId,username,name,color,avatar,
